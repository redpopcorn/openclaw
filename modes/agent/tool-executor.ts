...
export class ToolExecutor {
  ...
  public interactiveApproval = false;
  ...
  private async askPermission(message: string): Promise<boolean> {
    if (!this.interactiveApproval) return true;
  ...
  async createFile(rel: string, content: string): Promise<string> {
    if (!this.config.tools.allowFileCreation)
      throw new Error("File creation disabled");
    this.assertNotExcluded(rel, "create_file");
    const key = this.norm(rel);
    const approved = await this.askPermission(`Staging: Create new file "${key}"?`);
    if (!approved) throw new Error(`Action creation rejected by user for file: ${key}`);
    const abs = this.resolveSafe(rel);
  ...
  async modifyFile(rel: string, content: string): Promise<string> {
    ...
    const key = this.norm(rel);
    const approved = await this.askPermission(`Staging: Modify file "${key}"?`);
    if (!approved) throw new Error(`Action modification rejected by user for file: ${key}`);
    this.overlay.set(key, content);
    this.tracker.log({
      type: "file_modify",
      path: key,
  ...
  async deleteFile(rel: string): Promise<string> {
    ...
    const key = this.norm(rel);
    const approved = await this.askPermission(`Staging: Delete file "${key}"?`);
    if (!approved) throw new Error(`Action deletion rejected by user for file: ${key}`);
  ...
  async createFolder(rel: string): Promise<string> {
    if (!this.config.tools.allowFolderCreation)
      throw new Error("Folder creation disabled");
    this.assertNotExcluded(rel, "create_folder");
    const key = this.norm(rel);
    const approved = await this.askPermission(`Staging: Create folder "${key}"?`);
    if (!approved) throw new Error(`Action folder creation rejected by user: ${key}`);
    this.tracker.log({
      type: "folder_create",
      path: key,
      details: { after: key },
  ...
  async queueShell(command: string): Promise<string> {
    if (!this.config.tools.allowShellExecution)
      throw new Error("Shell execution disabled");
    const approved = await this.askPermission(`Staging: Run shell command "${command}"?`);
    if (!approved) throw new Error(`Action shell execution rejected by user for command: ${command}`);
    this.tracker.log({
      type: "tool_execute",
      path: "shell",
